...
import { LayoutGridIcon, MenuIcon, UserIcon, XIcon,  CalendarIcon, FileTextIcon, DollarSignIcon, SettingsIcon, User2Icon, ChevronRightIcon, LogOut, LogOutIcon } from 'lucide-react';
...
const Sidebar = () => {
    ...
    // clase mobile sidebar on router change
        ...
        <>
            ...
            <div className='flex-1 px-3 space-y-0.5 overflow-y-auto'>
                    {navItems.map((items) => {
                        ...
                        return (
                            <Link key={items.name} to={items.href} className={`group flex items-center gap-3 px-3 py-2.5 rounded-md text-[13px] font-medium transition-all duration-150 relative ${isActive ? "bg-indigo-500/12 text-indigo-300" : "text-slate-300 hover:text-white hover:bg-white/4"}`}>
                                {isActive && <div className='absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-5 rounded-r-full bg-indigo-500' />}
                                <items.icon  className={`w-[17px] h-[17px] shrink0 ${isActive ? "text-indigo-300" : "text-slate-400 group-hover:text-slate-300"}`}/>
            ...
            <div className='p-3 border-t border-white/6'>
                        <button onClick={handleLogout} className='flex items-center cursor-pointer gap-3 w-full px-3 py-2.5 rounded-md text-[13px] font-medium text-slate-400 hover:text-rose-400 hover:bg-rose-500/8 transition-all duration-150'>
